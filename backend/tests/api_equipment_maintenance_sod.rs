...
///
/// ⚠️ `recipient_role = 'approver'` 是**刻意加進斷言**的，不是照抄查詢條件：
/// 對帳（`services/notification/reconcile.rs`）的關卡分支全部靠這個標記篩選，
/// 產品端哪天不寫了，對帳那條分支會靜默失效，而只看 `kind`/`priority` 的斷言照樣綠。
/// 2026-08-26 實查測試庫可證這個欄位確實由產品路徑寫入（產品路徑產生的 16 筆全為
/// `approver`，NULL 的 13 筆全來自加這個標記之前的 commit）——但那是一次性人工驗證，
/// 要它不退化就得由測試鎖住。
