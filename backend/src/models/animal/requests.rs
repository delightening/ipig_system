...
pub struct UpdateAnimalRequest {
    ...
    // 注意：breed 本身不接受直接更新，但改 species_id 會連動重推 breed
    // （見 core::update::update / SpeciesLink::resolve_from_species），並非完全不可變。
...
// 疫苗/驅蟲：前端下拉含「其他」自訂文字選項，故不做封閉 enum 檢查，
// 僅防呆全空白字串與長度上限（`length(min=1)` 對純空白字串會誤放行，故用 trim 後檢查）
pub(crate) fn validate_non_blank_short_text(value: &str) -> Result<(), validator::ValidationError> {
    let trimmed = value.trim();
    if trimmed.is_empty() || trimmed.chars().count() > 100 {
        return Err(validator::ValidationError::new("blank_or_too_long"));
    }
    Ok(())
}
...
pub struct CreateVaccinationRequest {
    ...
    #[validate(custom(
        function = "validate_non_blank_short_text",
        message = "疫苗欄位不可為空白，且須在 100 字以內"
    ))]
    pub vaccine: Option<String>,
    #[validate(custom(
        function = "validate_non_blank_short_text",
        message = "驅蟲劑量欄位不可為空白，且須在 100 字以內"
    ))]
