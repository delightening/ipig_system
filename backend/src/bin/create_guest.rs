...
async fn main() -> Result<()> {
    ...
    // 支援 Docker Secrets（DATABASE_URL_FILE）和直接環境變數（DATABASE_URL）
    let database_url = if let Ok(file_path) = std::env::var("DATABASE_URL_FILE") {
        std::fs::read_to_string(&file_path)
            .map_err(|e| anyhow!("Failed to read DATABASE_URL_FILE '{}': {}", file_path, e))?
            .trim()
            .to_string()
    } else {
        std::env::var("DATABASE_URL").map_err(|_| anyhow!("Neither DATABASE_URL nor DATABASE_URL_FILE is set"))?
    };
    ...
    if let Some(role_id) = role_id {
        ...
        return Err(anyhow!("GUEST 角色不存在，請先確認 migration 030_guest_role.sql 已執行完成"));
    }
