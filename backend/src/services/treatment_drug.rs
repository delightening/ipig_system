...
impl TreatmentDrugService {
    ...
    /// 預設劑量單位須為可用單位成員（雙重保險：DB 亦有同義 CHECK 約束，
    /// 這裡先擋是為了回友善的 400 訊息，而非讓 constraint violation 裸露成 500）
    fn validate_default_unit(
        default_dosage_unit: Option<&String>,
        available_units: Option<&Vec<String>>,
    ) -> Result<(), AppError> {
        let Some(unit) = default_dosage_unit else {
            return Ok(());
        };
        let in_list = available_units.is_some_and(|units| units.iter().any(|u| u == unit));
        if !in_list {
    ...
    /// 表單傳來的空字串（或全空白）代表「未選擇」，正規化成 None——
    /// 否則會被 validate_default_unit 誤判成「設定了一個不存在的空單位」而擋下
    /// （2026-08 Qodo 審查發現：新增藥物一律 400，因前端未選單位時預設送出空字串）
    fn normalize_unit(unit: Option<String>) -> Option<String> {
        unit.filter(|u| !u.trim().is_empty())
    }
    ...
    pub async fn create(
        ...
        let default_dosage_unit = Self::normalize_unit(request.default_dosage_unit);
        Self::validate_default_unit(
            default_dosage_unit.as_ref(),
            request.available_units.as_ref(),
        )?;
    ...
    pub async fn update(
        ...
        // default_dosage_unit 三態語意：request 帶空字串＝使用者主動清空（須寫 NULL，
        // 不可回退成 existing 的舊值）；request 完全未帶欄位（None）＝不動此欄，沿用
        // existing；request 帶非空字串＝設成新值。前端 toggleUnit 取消勾選目前預設
        // 單位時會明確送出空字串，若在這裡被 .or(existing...) 蓋掉，會導致驗證繼續
        // 用「已被移出可用單位清單」的舊預設值去比對新清單而永遠擋下更新
        // （2026-08 Qodo 審查發現：無法清空/改掉預設單位）。
        let new_default_unit = match request.default_dosage_unit {
            Some(u) if u.trim().is_empty() => None,
            Some(u) => Some(u),
            None => existing.default_dosage_unit.clone(),
        };
        let new_available_units = request
            .available_units
            .as_ref()
            .or(existing.available_units.as_ref());
        Self::validate_default_unit(new_default_unit.as_ref(), new_available_units)?;
