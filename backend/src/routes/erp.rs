...
///
/// R84-14：原本 209 行全塞在單一 `routes()`，依領域拆成多個 builder 後
/// 只負責 `.merge()`——純結構重構，路由表（path/method/handler）逐條保持不變。
pub fn routes() -> Router<AppState> {
    Router::new()
        .merge(warehouse_routes())
        .merge(storage_location_routes())
...
fn storage_location_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/storage-locations",
