...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/hr/overtime/{id}/approve",
            post(handlers::approve_overtime),
        )
        ...
        // 靜態段先於 `{id}` 註冊：matchit 雖然本來就優先比對靜態段，
        // 但擺在前面可避免日後有人把它挪到 `{id}` 之後時 `calendar` 被當成 UUID 解析。
        .route("/hr/leaves/calendar", get(handlers::get_leave_calendar))
        .route(
            "/hr/leaves/{id}",
        ...
        // IACUC 委員名冊（依角色，與部門編制無關——委員會是跨部門的橫切面）
        .route(
            "/facilities/committee-members",
            get(handlers::list_committee_members),
        )
        .route(
