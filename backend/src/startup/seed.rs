...
/// R84-9 / R84-12 / R84-13：`DocType` 已移除 `DO` / `RM` / `SR` / `RTN`，
/// 但 DB enum 仍保留這四個值（選項 B）。
///
...
async fn verify_no_legacy_doc_types(pool: &sqlx::PgPool) -> Result<()> {
    let legacy_count: i64 = sqlx::query_scalar(
        r#"
        SELECT
            (SELECT count(*) FROM documents    WHERE doc_type::text IN ('DO', 'RM', 'SR', 'RTN'))
          + (SELECT count(*) FROM stock_ledger WHERE doc_type::text IN ('DO', 'RM', 'SR', 'RTN'))
        "#,
    )
    .fetch_one(pool)
    .await?;
    ...
    if legacy_count > 0 {
        tracing::error!(
            "[Schema] ⚠️ 發現 {} 筆 doc_type 為 DO/RM/SR/RTN 的舊資料——這四個值已從 Rust DocType 移除，             讀取這些列時 sqlx 會 decode 失敗（單據/流水查詢會 500）。             請改用現行單據類型回填，或評估恢復對應 enum 變體。",
            legacy_count
        );
        tracing::warn!("[Schema] ⚠ Schema integrity checked — legacy DO/RM/SR/RTN data detected");
    } else {
...
pub async fn seed_dev_users(pool: &sqlx::PgPool, config: &Config) -> Result<()> {
    ...
    let dev_users = [
        DevUser {
            email: "staff-lin@example.com",
            display_name: "佳蓉",
            roles: &["IACUC_STAFF", "EXPERIMENT_STAFF"],
        },
        DevUser {
            email: "staff-huang@example.com",
            display_name: "詩涵",
            roles: &["EXPERIMENT_STAFF"],
        },
        DevUser {
            email: "staff-hsieh@example.com",
            display_name: "俊傑",
            roles: &["EXPERIMENT_STAFF"],
        },
        DevUser {
            email: "staff-hung@example.com",
            display_name: "雅琪",
            roles: &["EXPERIMENT_STAFF", "WAREHOUSE_MANAGER"],
        },
        DevUser {
            email: "staff-chang@example.com",
            display_name: "振宏",
            roles: &["EXPERIMENT_STAFF"],
        },
        DevUser {
            email: "staff-cheng@example.com",
            display_name: "婉如",
            roles: &[
                "EXPERIMENT_STAFF",
                "WAREHOUSE_MANAGER",
