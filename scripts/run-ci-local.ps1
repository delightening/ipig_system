...
if (-not $SkipBackend) {
    ...
    try {
        ...
        # 整合測試護欄（backend/tests/common/test_db.rs，#37）只認這個變數、不
        # fallback 到 DATABASE_URL（開發機的 DATABASE_URL 可能指向 prod）。同值
        # 同步設定，比照 .github/workflows/ci.yml 的 backend-test job（#71
        # codeant-ai 審查抓到：本腳本先前只設 DATABASE_URL，會讓用共用護欄的
        # 測試全部 panic）。
