...
> ### 📖 關於這個 repo
>
> 這是 **只含程式碼的公開 repo**。專案的內部文件（規格、設計決策、進度台帳、
> 維運 runbook、合規追溯矩陣等）保存在另一個私有 repo，**不隨本 repo 發布**。
>
> 本文中提到 `docs/…` 路徑的地方標示為「內部文件」，指的就是那份私有 repo 的內容；
> 本 repo 內沒有對應檔案，這是刻意的，不是遺漏。
>
> 這麼切的理由是**最小揭露原則**：主機配置、備份位置、資料現況、災難復原佈署
> 等營運資訊，即使單項看來無害，合起來會構成一張完整的營運地圖。程式碼本身
> 開源沒有問題，那些不必要跟著出來。
...
子系統互動與請求生命週期（middleware → handler → service → repository → DB + audit-in-tx）詳見 docs/spec/architecture/ARCHITECTURE.md嚗?冽?隞塚?。
...
- 排程 job 每日驗證 chain 連續性，斷鏈時依內部 on-call 程序處理
...
雙向追溯（條款 → migration / service / test）詳見 docs/glp/traceability-matrix.md嚗?冽?隞塚?。下表為摘要（截至 R30 進行中）：
...
| §7 SOPs | `docs/glp/*-sop.md`嚗?冽?隞塚? 系列 | 🟡 |
...
正式部署（NAS / VPS / Cloudflare Tunnel）另有內部部署文件，不隨本公開 repo 發布。
...
詳見 docs/user/QUICK_START.md嚗?冽?隞塚?。
...
- docs/glp/R26_compliance_requirements.md嚗?冽?隞塚? — pattern 形成背景
- docs/glp/amendment-sop.md嚗?冽?隞塚? — 完整範本
...
- 詳見 RULES_BACKEND.md嚗?冽?隞塚? §4 與 RULES_FRONTEND.md嚗?冽?隞塚? §7（命名慣例）
...
1. **本 README**（你在這）
2. docs/README.md嚗?冽?隞塚? — docs hub，依角色分流
3. docs/spec/architecture/ARCHITECTURE.md嚗?冽?隞塚? — 部署圖、資料流、middleware → handler 完整 sequence
4. docs/PROGRESS.md嚗?冽?隞塚? — 各模組完成度 + §9 變更日誌
5. docs/glp/traceability-matrix.md嚗?冽?隞塚? — 稽核員雙向追溯入口
6. docs/reviews/codeReviewFindings.md嚗?冽?隞塚? — 三軸 code review 報告
...
| GLP / FDA 稽核員 | docs/glp/traceability-matrix.md → docs/glp/amendment-sop.md → 內部維運 runbook（不隨本公開 repo 發布） |
...
| 維運 / SRE | docs/ops/OPERATIONS.md → 內部部署手冊 → 內部災難復原 runbook（皆不隨本公開 repo 發布） |
