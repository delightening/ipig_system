...
**決策點**：`approve` 的 audit 是否要記錄「連帶 stock/accounting 變動」的 summary？建議在 `data_diff.after` 的 meta 欄位（或 `request_context`）加註 `{"stock_processed": true, "accounting_posted": true}`，但實際的 stock ledger / GL entries 仍由那兩個 service 自行 audit（R26-3 延伸）。
...
- 特別檢查：
  ...
  2. `approve` 的 audit row 早於 stock/accounting（tx 順序可從 created_at 驗證）
