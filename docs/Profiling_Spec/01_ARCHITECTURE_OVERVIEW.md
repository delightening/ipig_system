...
| 技術 | 說明 |
|------|------|
| Docker Compose | 5 核心服務 + prod/waf/monitoring overlay |
| Nginx | 前端靜態檔案服務 & API 反向代理（Brotli 壓縮）|
| WAF | ModSecurity + OWASP CRS（選用 docker-compose.waf.yml）|
| 監控 | Prometheus + Grafana + Alertmanager（選用 docker-compose.monitoring.yml）|
...
┌─────────────────────────────────────────────────────────────────┐
│                   PostgreSQL 16 資料庫                            │
│  19 遷移檔案 • 73+ 資料表 • 分割表 • 視圖 • pg_stat_statements  │
└─────────────────────────────────────────────────────────────────┘
```
...
|------------|----------|----------|------|
| Access Token | 可設定（預設 6h / 360 分鐘）| HttpOnly Cookie | API 認證 |
...
| 服務 | 映像檔 | 連接埠 | 說明 |
|------|--------|--------|------|
| `db` | postgres:16-alpine | 127.0.0.1:5433:5432 | PostgreSQL 資料庫（localhost-only）|
| `api` | 自建 Rust (cargo-chef) | 8000:8000 | 後端 API（非 root 運行）|
| `web` | 自建 Nginx (Brotli) | 8080:8080 | 前端靜態檔案 |
| `web-dev` | node:22-alpine | 5173:5173 | 開發環境 (Vite) |
| `db-backup` | postgres:16-alpine | — | 排程備份（cron + pg_dump + GPG）|
