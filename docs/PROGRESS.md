...
### 2026-03-14 採購入庫品項篩選強化
- ✅ **入庫邏輯嚴格化**：實作 GRN (採購入庫) 僅能選擇關聯 PO 之「已核准但未入庫」品項。
- ✅ **後端 API**：新增 `GET /api/documents/{id}/receipt-status` 介面，返還品項的採購、已收、剩餘數量。
- ✅ **前端整合**：`DocumentLineEditor` 於 GRN 模式下自動載入 PO 狀態，過濾已收足之品項，並自動填充單價與剩餘量。
