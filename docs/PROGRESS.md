...
| 9 | [最新變更動態](#9-最新變更動態) | 2026-03-20 R9-5 ERP/HR 整合測試覆蓋（4 spec / 26 tests） |
...
### 2026-03-20 R9-5 ERP/HR 整合測試覆蓋

- ✅ **庫存流水帳 E2E**：`stock-ledger.spec.ts` — 6 個測試涵蓋流水頁面標題、表格欄位、資料/空狀態、方向 badge、庫存查詢頁面載入與搜尋。
- ✅ **GRN 入庫單據 E2E**：`grn-document.spec.ts` — 9 個測試涵蓋單據管理頁面、三大類別 Tab（採購/銷貨/倉儲）、GRN 篩選、搜尋/狀態篩選、新增單據導航、單據建立頁面類型選擇與儲存按鈕。
- ✅ **出勤打卡 E2E**：`hr-attendance.spec.ts` — 7 個測試涵蓋出勤頁面、今日/歷史 Tab 切換、上班/下班打卡按鈕、日期範圍篩選、表格/空狀態、Excel 匯出、管理員查看所有人開關。
- ✅ **附件上傳下載 E2E**：`attachments.spec.ts` — 4 個測試涵蓋計畫書附件 Tab 可見性、附件表格/空狀態、上傳按鈕、動物詳情頁瀏覽。
