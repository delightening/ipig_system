...
- ✅ **後端**：Migration 011 新增 `animal_field_correction_requests` 表；`POST /animals/:id/field-corrections` 建立申請、`GET` 查詢該動物申請；`GET /animal/animal-field-corrections/pending` 列出待審、`POST /animal/animal-field-corrections/:id/review` 批准/拒絕。僅 admin 可審核。
