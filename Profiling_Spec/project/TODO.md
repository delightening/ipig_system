...
- [x] 1. 計畫種類與類型文字調整（含編號與 i18n）
  - **計畫類型 (Project Type)**:
    ...
    3. 產品上市前測試 (Pre-market Testing)
    4. 教學訓練 (Educational)
    5. 製造生物製劑 (Biologics Manufacturing)
    6. 其他 (Other (Explain))
  - **計畫種類 (Project Category)**:
    1. 醫學研究 (Medical Project)
    2. 農業研究 (Agricultural Project)
    3. 藥物及疫苗 (Drugs & Vaccines)
    4. 健康食品 (Dietary Supplements)
    5. 食品 (Food)
    6. 毒、化學品 (Toxics, Chemicals)
    7. 醫療器材 (Medical Materials)
    8. 農藥 (Pesticide)
    9. 動物用藥及疫苗 (Animal Drugs & Vaccines)
    10. 動物保健品、飼料添加物 (Animal Supplements & Feed Additives)
    11. (含藥)化妝品 (Cosmetics)
    12. 其他 (Other (Explain))
- [x] 2. 計畫類型與種類須加上編號（填寫監督報告需填寫編號而非名稱）
- [ ] 3. 計畫主持人與聯絡人處加上灰字說明工作內容（試驗單位主導人員、試驗執行主要聯絡人員）(在欄位裡面填灰字)
- [ ] 4. 試驗機構與設施位置鎖定並自動填寫本機構資料
- [ ] 5. 2.1處灰底字是否要修改成-(與農業部公布的新版本相同)摘要並簡單闡述本動物實驗計畫的意義及基本理由，此段描述不同於發表學術期刊及研討會
所需的摘要，而是提供給具有社會公正人士身分的 IACUC 委員進行審查的
重要根據，故應避免使醫學專用字彙與艱深科學概念(或盡可能說明清楚科
學意義)，與避免透露專利相關事宜。且需定義縮寫字詞，此處所撰寫內容
請勿超過一個A4頁面，相當於 250-500 個英文單字或 500-1000 個中文字。
- [ ] 6. 試驗物質照片或 SDS 上傳是否調整為非強制？(強制者標示星號)
- [ ] 7. 4.1.1 麻醉類型「非侵入式試驗」文字調整或刪除
- [ ] 8. 4.1.3 調整疼痛等級文字（農業部新版）
- [ ] 9. 人道終點加入公版文字（體重下降 20%、食慾不振等）或採用農業部文字
- [ ] 10. 4.2 加入化製處理公版文字（金海龍生物科技股份有限公司，管編：P6001213）
- [ ] 11. 5. 參考文獻格式參考農業部提供之格式
- [ ] 12. 6.2 術前準備文字修改（豬隻移至手術檯、趴姿氣管插管、電剪剃毛、接麻醉機）

- [ ] 13. 6.5 術中監控加入公版文字（依 TU-03-09-00 SOP 進行，監控心跳、呼吸、體溫等）
- [ ] 14. 6.7 增加「否」選項，且「是」的說明欄位需正確顯示
- [ ] 15. 6.10 依 6.8 選項自動產生藥物清單，或列出所有藥物供計畫負責人刪除
- [ ] 16. 6.10 給藥目的後方多餘欄位移除或顯示正確說明
- [ ] 17. 7. 飼養環境更名為「動物飼養場所」，並新增「動物來源」欄位
- [ ] 18. 8. 試驗人員新增時姓名欄位重複問題修復，且訓練班說明文字需顯示
