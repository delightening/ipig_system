...
export function MyProjectsPage() {
  ...
  // R89-8：完整 change_status（IACUC_CHAIR/IACUC_STAFF/admin）不限本人；
  // close_own（PI/SD）僅限計畫擁有人，用後端同一份定義的 can_edit 判斷是不是本人
  // （見 my_protocols.rs::my_protocols_sql，pi_user_id / study_director_user_id /
  // user_protocols PI 委派三者之一）。
  const canCloseProject = (project: ProtocolListItem) =>
    hasPermission(PERMISSIONS.AUP_PROTOCOL_CHANGE_STATUS) ||
    (hasPermission(PERMISSIONS.AUP_PROTOCOL_CLOSE_OWN) && !!project.can_edit)
            ...
            const renderActions = (project: ProtocolListItem) => (
              <>
                ...
                <GuestHide>
                  ...
                  {(project.status === 'APPROVED' || project.status === 'APPROVED_WITH_CONDITIONS') && canCloseProject(project) && (
                    <Button
                      variant="outline"
                      size="sm"
