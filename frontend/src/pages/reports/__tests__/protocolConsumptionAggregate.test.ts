...
describe('crossTabCsv', () => {
  ...
  it('表頭帶 sku，並把單位標在品名後面', () => {
    const header = crossTabCsv(tab, uom).split('\r\n')[0]
    expect(header).toBe('"計畫編號","CON-GLV-001 無菌手套 6號(雙)","CON-OTH-022 滴管(包)"')
  })
  ...
  it('🔴 同名同單位但不同 sku 的品項，表頭必須分得出來', () => {
    // products 只保證 sku 唯一，(name, base_uom) 沒有這個保證。
    // 不加 sku 的話兩欄表頭會一模一樣，而欄位是分開的——下游對不回是哪一個。
    const dupTab = buildCrossTab([
      row({ protocol_id: PA, product_id: GLOVE, product_sku: 'CON-GLV-001', qty_base: '10' }),
      row({
        protocol_id: PA,
        product_id: DROPPER,
        product_sku: 'CON-GLV-002',
        product_name: '無菌手套 6號', // 同名
        base_uom: '雙', // 同單位
        qty_base: '4',
      }),
    ])
    const [header, dataLine] = crossTabCsv(dupTab, uom).split('\r\n')
    expect(header).toBe('"計畫編號","CON-GLV-001 無菌手套 6號(雙)","CON-GLV-002 無菌手套 6號(雙)"')
    // 兩欄確實是不同的品項，數字沒有被合併
    expect(dataLine).toBe('"P-001","10","4"')
    // 而且兩個欄名不相同——這正是加 sku 要解決的事
    const cols = header.split(',')
    expect(new Set(cols).size).toBe(cols.length)
  })
})

describe('交叉表的規模上限（MAX_CROSS_CELLS）', () => {
  const uom = (u: string) => u

  /**
   * 對角線分佈：N 列，每一列是一個「只出現一次的案件」配一個「只出現一次的品項」。
   * 這就是 ROW_LIMIT 擋不住的形狀——列數只有 N，格數卻是 N²。
   */
  function diagonal(n: number) {
    return Array.from({ length: n }, (_, i) =>
      row({
        protocol_id: `protocol-${i}`,
        protocol_no: `P-${String(i).padStart(4, '0')}`,
        product_id: `product-${i}`,
        product_sku: `SKU-${String(i).padStart(4, '0')}`,
        product_name: `品項 ${i}`,
        qty_base: '1',
      })
    )
  ...
  it('🔴 扁平列數在 ROW_LIMIT 之內，格數仍可能遠超過上限', () => {
    const tab = buildCrossTab(diagonal(320))
    // 只有 320 列——連 ROW_LIMIT（1000）都沒碰到，truncated 不會亮
    expect(tab.cells.size).toBe(320)
    // 但格數是 320 × 320
    expect(crossTabCellCount(tab)).toBe(102_400)
    expect(crossTabCellCount(tab)).toBeGreaterThan(MAX_CROSS_CELLS)
    expect(crossTabTooLarge(tab)).toBe(true)
  })

  it('超過上限時 crossTabCsv 丟錯，不會默默組出一個巨大的字串', () => {
    const tab = buildCrossTab(diagonal(320))
    expect(() => crossTabCsv(tab, uom)).toThrow(/超過 100000 的匯出上限/)
  })

  it('上限內照常運作，沒有把正常情況一起擋掉', () => {
    const tab = buildCrossTab(diagonal(10))
    expect(crossTabCellCount(tab)).toBe(100)
    expect(crossTabTooLarge(tab)).toBe(false)
    expect(crossTabCsv(tab, uom).split('\r\n')).toHaveLength(11) // 表頭 + 10 列
  })

  it('邊界：剛好等於上限不算超過', () => {
    // 316² = 99,856 ≤ 100,000；317² = 100,489 > 100,000
    expect(crossTabTooLarge(buildCrossTab(diagonal(316)))).toBe(false)
    expect(crossTabTooLarge(buildCrossTab(diagonal(317)))).toBe(true)
  })
})
