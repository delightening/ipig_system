...
// 品類定義（與 ProductsPage 一致）
const CATEGORIES = [
  { code: 'DRG', name: '藥品', subcategories: [{ code: 'ABX', name: '抗生素' }, { code: 'ANL', name: '止痛藥' }, { code: 'VIT', name: '維生素' }, { code: 'OTH', name: '其他藥品' }] },
  { code: 'MED', name: '醫材', subcategories: [{ code: 'SYR', name: '注射器材' }, { code: 'BND', name: '敷料繃帶' }, { code: 'GLV', name: '手套' }, { code: 'OTH', name: '其他醫材' }] },
  { code: 'CON', name: '耗材', subcategories: [{ code: 'GLV', name: '手套' }, { code: 'GAU', name: '紗布敷料' }, { code: 'CLN', name: '清潔消毒' }, { code: 'TAG', name: '標示耗材' }, { code: 'LAB', name: '實驗耗材' }, { code: 'OTH', name: '其他耗材' }] },
  { code: 'CHM', name: '化學品', subcategories: [{ code: 'RGT', name: '試劑' }, { code: 'SOL', name: '溶劑' }, { code: 'STD', name: '標準品' }, { code: 'OTH', name: '其他化學品' }] },
  { code: 'EQP', name: '設備', subcategories: [{ code: 'INS', name: '儀器' }, { code: 'TOL', name: '工具' }, { code: 'PRT', name: '零件' }, { code: 'OTH', name: '其他設備' }] },
  { code: 'LAB', name: '耗材 (LAB)', subcategories: [{ code: 'TUB', name: '試管' }, { code: 'PIP', name: '吸管' }, { code: 'PLT', name: '培養皿' }, { code: 'OTH', name: '其他耗材' }] },
]
...
export function ProductEditPage() {
  ...
  useEffect(() => {
    if (product) {
      ...
      setSubcategoryCode(product.subcategory_code ?? '')
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      return api.put(`/products/${id}`, {
        ...
        pack_qty: packQty === '' ? undefined : Number(packQty),
        ...
        <div className="grid gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2">
                <Label>品類</Label>
                <Select value={categoryCode} onValueChange={(v) => { setCategoryCode(v); setSubcategoryCode('') }}>
                  <SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="packUnit">包裝單位</Label>
                  <Select value={packUnit} onValueChange={setPackUnit}>
                    <SelectTrigger id="packUnit">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="packQty">包裝量</Label>
                  <Input
                    id="packQty"
                    type="number"
                    min={1}
                    value={packQty}
                    onChange={(e) => setPackQty(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
                    placeholder="例：40"
                  />
                </div>
              </div>
